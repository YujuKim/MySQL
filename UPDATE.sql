...
-- 2. id가 3인 사람의 메뉴를 감바스로 변경
update `order`
set product_name = '감바스'
where id =3;

-- 3. id가 5인 주문의 주문일자를 '2022-01-05'로 수정
-- update 구문에 DATE값 입력 시엔 작은 따옴표로 감싸주기! (프로그램마다 상이할 수 있음)
UPDATE `order` 
SET 
...
-- 테이블 구조 변경해보기 : 컬럼 추가
ALTER TABLE `order` ADD column address VARCHAR(100); -- 'column'은 생략 가능
...
-- 주문 날짜가 '2022-02-28'이고 상품명이 '초밥'인 주문의 주문 수량을 5개로 설정
UPDATE `order` 
SET 
    quantity = '5'
WHERE
